...
const Spotlight = () => {
	...
	// Fetch featured portfolio items from Sanity
	useEffect(() => {
		async function fetchSpotlightItems() {
			try {
				const items = await client.fetch(
					groq`*[_type == 'portfolio.item' && featured == true] | order(publishDate desc) [0...10] {
            'name': metadata.title,
	...
	useEffect(() => {
		...
		// Wait for spotlight items to be loaded
